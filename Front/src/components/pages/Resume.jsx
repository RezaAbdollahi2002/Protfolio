...
const Resume = () => {
  const [email,setEmail] = useState("apa6323@psu.edu");
  const [resume,setResume] = useState("");

  useEffect(() => {
    const getResume = async () => {
      try {
        const res = await axios.get(`/api/developer/get/resume/${email}`);
        setResume(res.data);
  ...
  return (
    <div className="mx-auto h-screen  pb-6 ">
      <div className="flex justify-center gap-x-2">
        ...
        <button>
          <a href={`/api/${resume}`}><FaDownload className="w-7 h-7 text-white ml-1 mt-5" /></a>
        </button>
      </div>
